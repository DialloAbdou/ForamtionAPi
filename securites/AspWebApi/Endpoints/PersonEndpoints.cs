...
namespace AspWebApi.Endpoints
{
    public static class PersonEndpoints
    {
        public static RouteGroupBuilder MaPersonEndpoints(this RouteGroupBuilder group)
        {
            ...
            group.MapGet("/{id}", GetById).WithTags("Application Personnel");


            group.MapPut("/{id}", GetPut
            ).Produces(204)
             .Produces(404)
             .WithTags("Application Personnel");

            group.MapDelete("/{id}",GetDeleted
                        ...
                        CancellationToken token)
        {
           ...
           var personOutput=  await service.AddPersonne(personne);
            return Results.Ok(personOutput);
